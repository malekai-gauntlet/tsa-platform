...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log('✅ Session data retrieved:', {
      ...
      expires_in_minutes: Math.round((new Date(sessionData.expires_at).getTime() - Date.now()) / (1000 * 60))
    })
...
export async function POST(request: NextRequest) {
  try {
    const { email, invitationToken, ttlMinutes, encryptSensitiveData = true } = await request.json()
    ...
    const dataToStore = encryptSensitiveData ? encryptData(initialStepData) : JSON.stringify(initialStepData)
...
export async function PUT(request: NextRequest) {
  try {
    ...
    const dataToStore = isEncrypted ? encryptData(updatedStepData) : JSON.stringify(updatedStepData)
    ...
    const sessionData: SessionData = {
      ...
      current_step: currentStep || progress.currentStep?.toLowerCase().replace('_', '-') || 'personal-info',
