...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data: profile } = await client.models.Profile.create({
      ...
      preferences: data.profile?.preferences ? JSON.stringify(data.profile.preferences) : JSON.stringify({
        ...
        sport: data.sport,
        grade_levels: data.student_grade_levels || data.grade_levels_served || data.school?.gradeLevels || [],
        academic_year: data.academic_year
      }),
      address: data.profile?.address ? JSON.stringify(data.profile.address) : JSON.stringify({
    ...
    if (data.invitation_token || data.invitation_id) {
      try {
        ...
        if (data.invitation_token) {
          ...
          const { data: invitationById } = await client.models.Invitation.get({ id: data.invitation_id })
