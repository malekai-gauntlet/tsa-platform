...
export function useOnboardingState({
  ...
  const [formData, setFormData] = useState<OnboardingFormData>({
    ...
    gender: '',
    hispanicLatinoEthnicity: undefined,
    races: [],
    emergencyContact: '',
    experience: '',
    certifications: [],
    ...
    try {
      ...
      if (isInBypassMode && !storedInvitationData) {
        ...
        setFormData(prev => ({
          ...
          schoolName: simulatedInvitationData.schoolName,
        ...
        setFormData(prev => ({
          ...prev,
          // Map invitation fields to form fields
          ...
          schoolName: storedInvitationData.schoolName || '',
  ...
  const validateStep = useCallback(() => {
    ...
    requiredFields.forEach(field => {
      // Convert legacy snake_case field names to camelCase for lookup
      const camelCaseField = field.replace(/_([a-z])/g, (_, letter) => letter.toUpperCase());
      const value = formData[camelCaseField as keyof OnboardingFormData];
    ...
    if (requiredFields.includes('birth_date') && formData.birthDate) {
