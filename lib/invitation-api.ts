...
class CoachInvitationAPI {
  ...
  async validateInvitation(token: string): Promise<InvitationValidationResponse> {
    ...
    const transformedInvitation: InvitationData = {
      ...
      phone_formatted: result.invitation.phone_formatted,
      school_name: result.invitation.schoolName,
  ...
  async getOnboardingProgress(email: string, invitationToken?: string): Promise<OnboardingProgress | null> {
    ...
    return {
      ...
      completed_steps: (result.completed_steps || []).filter((step): step is string => step !== null),
      step_data: typeof result.step_data === 'object' ? result.step_data as Record<string, any> : {},
  ...
  async updateOnboardingProgress(
    ...
    return await onboardingOperations.updateOnboardingProgress(email, currentStep, stepData, completedSteps, invitationToken);
  }
