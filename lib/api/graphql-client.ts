...
// Type for onboarding steps to avoid repetition
type OnboardingStep =
  | 'PERSONAL_INFO'
  | 'ROLE_EXPERIENCE'
  | 'SCHOOL_SETUP'
  ...
  async validateInvitation(token: string): Promise<InvitationValidationResponse> {
    try {
      ...
      const invitationData = {
        ...
        location: city ? `${city}, ${state}`.trim() : state,
...
export const onboardingOperations = {
  async getOnboardingProgress(
    ...
    try {
          ...
          if (result.valid && result.invitation) {
            ...
            if (newProgress) {
              return {
                ...
                current_step:
                  newProgress.currentStep?.toLowerCase().replace('_', '-') ?? 'personal-info',
                completed_steps:
                  newProgress.completedSteps?.filter((x): x is string => typeof x === 'string') ??
                  [],
                step_data: (newProgress.stepData as Record<string, any>) ?? {},
                last_updated: newProgress.lastUpdated ?? new Date().toISOString(),
                invitation_based: newProgress.invitationBased ?? false,
                invitation_id: newProgress.invitationId ?? undefined,
              };
            }
            return null;
          }
      ...
      return {
        ...
        current_step:
          existingProgress.currentStep?.toLowerCase().replace('_', '-') ?? 'personal-info',
        completed_steps:
          existingProgress.completedSteps?.filter((x): x is string => typeof x === 'string') ?? [],
        step_data: (existingProgress.stepData as Record<string, any>) ?? {},
        last_updated: existingProgress.lastUpdated ?? new Date().toISOString(),
        invitation_based: existingProgress.invitationBased ?? false,
        invitation_id: existingProgress.invitationId ?? undefined,
      };
    } catch (error) {
  ...
  async updateOnboardingProgress(
    ...
    try {
      ...
      const formattedCurrentStep = currentStep.toUpperCase().replace('-', '_') as OnboardingStep;
      ...
      if (!progress?.length) {
        ...
        const existingProgress = progress[0];
        // Create a unique array by using filter instead of Set
        const existingSteps = existingProgress.completedSteps ?? [];
        const updatedCompletedSteps = [...existingSteps, ...completedSteps].filter(
          (step, index, self) => step !== null && self.indexOf(step) === index
        );
        ...
        await client.models.OnboardingProgress.update({
          ...
          stepData: {
            ...((existingProgress.stepData as Record<string, any>) || {}),
            ...stepData,
          },
          lastUpdated: new Date().toISOString(),
        });
    ...
    try {
      ...
      if (data.schoolName) {
        const { data: school } = await client.models.EducationOrganization.create({
          ...
          educationOrganizationId: Math.floor(Math.random() * 1000000), // Generate a random ID
        });
        schoolId = school?.id;
      }
      ...
      if (progressRecord) {
        await client.models.OnboardingProgress.update({
          ...
          completedSteps: [
            // Create a unique array by using filter instead of Set
            ...(progressRecord.completedSteps ?? []),
            ...data.completedSteps,
            'COMPLETE',
          ].filter((step, index, self) => step !== null && self.indexOf(step) === index),
          lastUpdated: new Date().toISOString(),
        });
      }
