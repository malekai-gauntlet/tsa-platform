...
function getBackendEnvironmentConfig() {
  const apiKey = process.env.AMPLIFY_DATA_GRAPHQL_API_KEY || process.env.NEXT_PUBLIC_AMPLIFY_DATA_GRAPHQL_API_KEY
  const endpoint = process.env.AMPLIFY_DATA_GRAPHQL_ENDPOINT || process.env.NEXT_PUBLIC_AMPLIFY_DATA_GRAPHQL_ENDPOINT
  const region = process.env.AMPLIFY_REGION || process.env.NEXT_PUBLIC_AMPLIFY_REGION || 'us-east-1'
