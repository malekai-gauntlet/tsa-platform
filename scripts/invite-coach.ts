...
class TSACoachInvitationService {
   ...
   */
  async createCognitoUser(data: CoachInvitationData, temporaryPassword: string): Promise<{ success: boolean; userId?: string; error?: string }> {
    try {
   ...
   */
  async createInvitationRecord(data: CoachInvitationData, token: string, userId: string): Promise<{ success: boolean; invitationId?: string; error?: string }> {
    try {
      const { firstName, lastName } = this.parseFullName(data.name)
      const formattedPhone = this.formatPhoneNumber(data.cell)
   ...
   */
  async sendInvitationEmail(data: CoachInvitationData, token: string, temporaryPassword: string): Promise<{ success: boolean; messageId?: string; error?: string }> {
    try {
      const { firstName, lastName } = this.parseFullName(data.name)
   ...
   */
  async inviteCoach(data: CoachInvitationData, options: { dryRun?: boolean; testEmailOnly?: boolean } = {}): Promise<InvitationResult> {
    ...
    try {
      ...
      if (options.testEmailOnly) {
        ...
        if (emailResult.success) {
          ...
          console.log(`   💡 Note: This was a test email only - no user or database record was created`)
          return { success: true }
        } else {
...
`)
  ...
  try {
    ...
    if (options.dryRun) {
      ...
      console.log('📧 TEST EMAIL MODE - Sending email only (no user creation or database operations)')
    }
